...
class LayeredHistogramBuilder:
    """Builder class for creating a layered histogram chart using Altair.
    ...
    def __init__(self, histogram: pchrt.LayeredHistogramChart) -> None:
        """Initialize the < LayeredHistogramBuilder > with a < Histogram > object.

        Parameters:
            histogram (pchrt.LayeredHistogramChart): Histogram object containing chart attributes.
    ...
    def x(
        ...
        Returns:
            LayeredHistogramBuilder: The current builder instance.
        """
    ...
    def x2(self, shorthand: str) -> "LayeredHistogramBuilder":
        """Set x2-axis attributes.

        Parameters:
            shorthand (str | None): The shorthand for the x2-axis encoding.

        Returns:
            LayeredHistogramBuilder: The current builder instance.
        """

        if self._obj.encode.x2 is None:
            raise ValueError("To assign < x2 > attributes, the object must not be None.")
        self._obj.encode.x2.shorthand = shorthand
    ...
    def order(self, shorthand: str) -> "LayeredHistogramBuilder":
        """Set order attributes.

        Parameters:
            shorthand (str | None): The shorthand for the order encoding.

        Returns:
            LayeredHistogramBuilder: The current builder instance.
        """

        if self._obj.encode.order is None:
            raise ValueError("To assign < order > attributes, the object must not be None.")
        self._obj.encode.order.shorthand = shorthand

        return self

    def tooltips(self, tooltips: Sequence[pschm.Tooltip]) -> "LayeredHistogramBuilder":
    ...
    def build(self) -> alt.LayerChart:
        ...
        # Chart attributes
        enc: pchrt.LayeredHistogramChartEncodings = self._obj.encode
        if enc.x.axis is None:
            raise ValueError("To assign < x.axis > attributes, the object must not be None.")
        ...
        # Bin data into two fields x_start & x_end (note use of walrus operator)
        # Aggregate counts per bin and per category and order to achieve layering
        chart = (
            alt.Chart(self._obj.frame)
            .transform_bin(
                as_=[
                    x_start := bhlp.remove_shorthand_type(enc.x.shorthand),
                    x_end := bhlp.remove_shorthand_type(enc.x2.shorthand),
                ],
                field=enc.x.axis.title,
                bin=alt.BinParams(
            ...
            .transform_aggregate(
                ...
                groupby=[
                    x_start,
                    x_end,
                    bhlp.remove_shorthand_type(enc.color.shorthand),
                    bhlp.remove_shorthand_type(enc.order.shorthand),
                ],
                mean_late=f"mean({enc.x.axis.title})",
            )
            .transform_calculate(
                bin_range=f"datum.{x_start} + ' - ' + datum.{x_end}"
            )  # bin_range=< JavaScript expression >
        )
        ...
        encodings = {
            "x": alt.X(
                ...
                bin="binned",  # binned above
                scale=bhlp.build_scale(enc.x.scale),
            ),
            "x2": alt.X2(shorthand=enc.x2.shorthand),
            "y": alt.Y(
            ...
            "order": alt.Order(shorthand=enc.order.shorthand),
            "tooltip": bhlp.create_tooltips(enc.tooltip),
        }

        # Bar chart (binned histogram)
        bar_layer = chart.mark_bar(
