...
class ChannelDescriptor(Enum):
    ...
    Y = EncodingDescriptor("Fiscal Year Quarter:N", "Period")
    COLOR = EncodingDescriptor("Color:N")
    OUTLIER = EncodingDescriptor("outliers:Q", "Average Minutes Late")
...
class TickMarkProperties:
    ...
    color: str | None = "#ffffff"  # median line
    size: float | None = 20.0


@dataclass(kw_only=True)
class BoxplotPreAggChart:
    ...
    Fields:
        ...
        properties (chrt.ChartProperties): chart properties
    """
    ...
    median: pmrk.TickMarkProperties = field(default_factory=TickMarkProperties)
    outlier: pschm.PrimaryChannel = field(init=False)
    properties: pchrt.ChartProperties = field(init=False)

    def __post_init__(self) -> None:
        self.outlier = chrt.PrimaryChannel(
            shorthand=ChannelDescriptor.OUTLIER.value.shorthand,
            scale=None,  # hex codes already exist in chart_data Color column
            title=ChannelDescriptor.OUTLIER.value.title,
        )
        self.properties = chrt.ChartProperties(height=350, width=680, padding=15, title=self.title)
